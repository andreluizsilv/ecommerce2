...
def enviar_email_compra(pedido):
    enviar_email_status(pedido, "Pedido aprovado", "Parabéns! Seu pedido foi aprovado.")


def enviar_email_status(pedido, assunto_base, mensagem):
    email = pedido.cliente.email
    assunto = f"{assunto_base}: {pedido.id}"
    corpo = f"""{mensagem}
...
def enviar_email_pagamento(pedido, status):
    if status == "success":
        enviar_email_status(pedido, "Pagamento aprovado", "Seu pagamento foi aprovado com sucesso.")
    elif status == "pending":
        enviar_email_status(pedido, "Pagamento pendente", "Seu pagamento está pendente. Aguardando confirmação.")
    elif status == "failure":
        enviar_email_status(pedido, "Pagamento falhado", "Seu pagamento falhou. Tente novamente.")
