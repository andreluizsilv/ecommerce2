...
from .models import *
import uuid
from .utils import filtrar_produtos, preco_minimo_maximo, ordenar_produtos, enviar_email_pagamento, exportar_csv
...
def finalizar_pagamento(request):
    ...
    if status == "success":
        ...
        pedido.save()
        pagamento.save()
        enviar_email_pagamento(pedido, "success")
        if request.user.is_authenticated:
        ...
        pagamento.save()
        enviar_email_pagamento(pedido, "pending")
        context = {"mensagem": "Seu pagamento está pendente. Aguardando confirmação."}
        return render(request, "pagamento_pendente.html", context)
    elif status == "failure":
        pagamento.aprovado = False
        pagamento.save()
        enviar_email_pagamento(pedido, "failure")
        context = {"mensagem": "Seu pagamento falhou. Tente novamente."}
        return render(request, "pagamento_falhado.html", context)
    else:
